...
import com.froad.vo.trans.GoodsCarryRackTrans;
 ...
 * @date 2013-2-19  
 * @version 1.0
 * 商品提现架 client service  ActionSupport
 */
public class GoodsCarryRackActionSupport {
	private static Logger logger = Logger.getLogger(GoodsCarryRackActionSupport.class);
	private GoodsCarryRackService goodsCarryRackService;
//	private GoodsExchangeRackService goodsCarryRackService;
	...
	public TransRule getExchCurrencyRule(){
		...
		if(!Assert.empty(cache))
			rules_ruleType=cache.get("rules-ruleType");
		List<TransRule> pointsExchCurrencyRuleList=null;
		if(!Assert.empty(rules_ruleType))
			pointsExchCurrencyRuleList=rules_ruleType.get(RuleType.WITHDRAW.getValue());
		if(!Assert.empty(pointsExchCurrencyRuleList))
			pointsExchCurrencyRule=pointsExchCurrencyRuleList.get(0);
		return pointsExchCurrencyRule;
	}
	
	public GoodsCarryRack selectById(String id) {
		return goodsCarryRackService.selectById(Integer.parseInt(id));
	}
	
	public GoodsCarryRack getGoodsCarryRacks(GoodsCarryRack queryCon){
		GoodsCarryRack result=null;
		try {
			result = goodsCarryRackService.getGoodsCarryRackListByPager(queryCon);
		} catch (Exception e) {
	...
	public GoodsCarryRack getGoodsCarryRack(String goodsId){
		GoodsCarryRack result=null;
		List<GoodsCarryRack> GoodsCarryRackList=new ArrayList();
		try {
			GoodsCarryRackList =  goodsCarryRackService.getGoodsCarryRackByGoodsId(goodsId);
		} catch (Exception e) {
			// TODO: handle exception
			logger.error("查询积分兑换的交易品信息失败!", e);
		}
		result=GoodsCarryRackList.get(0);
		return result;
	}
	
	public GoodsCarryRack getGoodsCarryRacksById(String id){
		GoodsCarryRack result=null;
		if(id==null)
			return result;
		try {
			result = (GoodsCarryRack) goodsCarryRackService.selectById(Integer.parseInt(id));
		} catch (Exception e) {
			// TODO: handle exception
			logger.error("查询积分兑换的交易品失败!", e);
		}
	...
	//创建积分兑换交易  客户端
//	public Trans createPointsExchProductTrans(ClientGoodsCarryRack goodsRack, Trans trans,
//			TransDetails transDetails, Buyers buyer,User user,UserCertification userCertification,
//			TransAddtionalInfoVo transAddtionalInfoVo, String clientType){
//		GoodsCarryRackTrans goodsCarryRackTrans=new GoodsCarryRackTrans();
//		
//		if(goodsRack==null){
//			logger.error("客户版的ClientGoodsCarryRack为空");
//			return null;
//		}
//		
//		com.froad.client.clientGoodsCarryRack.Goods goods=goodsRack.getGoods();
//		if(goods==null){
//			logger.error("客户版的ClientGoodsCarryRack的good为空！");
//			return null;
//		}
//		
//		if(goodsRack==null||Assert.empty(goodsRack.getGoods().getMerchantId())){
//			logger.error("客户版的ClientGoodsCarryRack的merchantID为空！");
//			return null;
//		}
//		
//			
...
//		goodsCarryRackTrans.setGoods(goodsInTrans);
//		goodsCarryRackTrans.setMerchantId(goodsRack.getGoods().getMerchantId());
//		goodsCarryRackTrans.setId(goodsRack.getId());
//		goodsCarryRackTrans.setIsCash(goodsRack.getIsCash());
//		goodsCarryRackTrans.setIsBankPoint(goodsRack.getIsBankPoint());
//		goodsCarryRackTrans.setIsBankpointCash(goodsRack.getIsBankpointCash());
//		goodsCarryRackTrans.setIsFftPoint(goodsRack.getIsFftPoint());
//		goodsCarryRackTrans.setIsFftpointCash(goodsRack.getIsFftpointCash());
//		goodsCarryRackTrans.setIsFftpointcashRatioPricing(goodsRack.getIsFftpointcashRatioPricing());
//		goodsCarryRackTrans.setIsBankpointcashRatioPricing(goodsRack.getIsBankpointcashRatioPricing());
//		
//		goodsCarryRackTrans.setCashPricing(goodsRack.getCashPricing());
//		goodsCarryRackTrans.setBankPointPricing(goodsRack.getBankPointPricing());
//		goodsCarryRackTrans.setBankpointCashPricing(goodsRack.getBankpointCashPricing());
//		goodsCarryRackTrans.setFftPointPricing(goodsRack.getFftPointPricing());
//		goodsCarryRackTrans.setFftpointCashPricing(goodsRack.getFftpointCashPricing());
//		goodsCarryRackTrans.setFftpointcashRatioPricing(goodsRack.getFftpointcashRatioPricing());
//		goodsCarryRackTrans.setBankpointcashRatioPricing(goodsRack.getBankpointcashRatioPricing());
//		
//		goodsCarryRackTrans.setSellerId(String.valueOf(goodsRack.getSellerId()));
//		return createPointsExchProductTrans( goodsCarryRackTrans,  trans,
//				 transDetails,  buyer,  user,  userCertification,
//				 transAddtionalInfoVo,  clientType);
//	}
	
	
	//创建积分兑换交易  PC端
//	public Trans createPointsExchProductTrans(GoodsCarryRack goodsRack, Trans trans,
//			TransDetails transDetails, Buyers buyer, User user, UserCertification userCertification,
//			TransAddtionalInfoVo transAddtionalInfoVo, String clientType){
//		GoodsCarryRackTrans goodsCarryRackTrans=new GoodsCarryRackTrans();
//		
//		if(goodsRack==null){
//			logger.error("PC版的GoodsCarryRack为空");
//			return null;
//		}
//		
//		com.froad.client.goodsCarryRack.Goods goods=goodsRack.getGoods();
//		if(goods==null){
//			logger.error("PC版的GoodsCarryRack的good为空！");
//			return null;
//		}
//		if(goodsRack==null||Assert.empty(goodsRack.getGoods().getMerchantId())){
//			logger.error("PC版的GoodsCarryRack的merchantID为空！");
//			return null;
//		}
//			
//		Goods goodsInTrans=new Goods();
...
//		goodsCarryRackTrans.setBankpointcashRatioPricing(goodsRack.getBankpointcashRatioPricing());
//		goodsCarryRackTrans.setSellerId(String.valueOf(goodsRack.getSellerId()));
////		goodsCarryRackTrans.setGroupPrice(goodsRack.getGroupPrice());
//		
//		return createPointsExchProductTrans( goodsCarryRackTrans,  trans,
//				 transDetails,  buyer,  user,  userCertification,
//				 transAddtionalInfoVo,  clientType);
...
//	public Trans createPointsExchProductTrans(GoodsCarryRackTrans goodsRack,
//			Trans trans,TransDetails transDetails,Buyers buyer,User user,
...
//				PayMethod payMethod = PayMethod.getPayMethod(trans
//						.getPayMethod());
...
//			updateTransAdditionalInfo(user,trans, transDetails, goodsRack,
//					transAddtionalInfoVo);
//			
//			BuyerChannel buyersDepositChannel =null;
//			
//			
//			//获取买家支付渠道
//			if(buyer!=null){
//				List<BuyerChannel> buyersDepositChannelList = buyer.getBuyerChannelList();
//				if(!Assert.empty(buyersDepositChannelList))
//					buyersDepositChannel = buyersDepositChannelList.get(0);
//			}
//			
//			try {
//				//继续组装交易数据（组织一条交易的相关pay信息）
//				transServiceInClient.countTansferInfoTran(trans, seller1,
//						sellerDepositChannel1, buyer,buyersDepositChannel,user,userCertification, 
...
//	}
	...
	//更新交易信息
	private void updateTransBaseInfo(Trans trans,TransDetails transDetails,GoodsCarryRackTrans goodsRack,Seller seller,Buyers buyer){
...
//		trans.setMerchantId(seller.getMerchantId());
//		trans.setSellerType(seller.getSellerType());
		...
		if(GoodCategory.Points_Exch_Currency.getValue().equals(goodsRack.getGoods().getGoodsCategory().getName()))
			trans.setTransType(TransType.Trans_Points_Exch_Currency.getValue());
		else
			trans.setTransType(TransType.Trans_Points_Exch_Product.getValue());
	...
	//更新交易信息
	private void updateTransAdditionalInfo(User user,Trans trans,TransDetails transDetails,GoodsCarryRackTrans goodsRack,TransAddtionalInfoVo transAddtionalInfoVo){
...
//	private void price(GoodsCarryRackTrans goods,TransDetails transDetails,PayMethod payMethod) throws PriceError, PointsUseError{
...
////			}
//			bankPoints=Integer.valueOf(bankPointsStr);
...
//			if(index==-1||index==0||index==fftpointsAndCurrency.length())
//				throw new PriceError("交易品定价有误,请检查交易品id:"+goods.getId()+"的定价");
...
//			if(index2==-1||index2==0||index2==bankpointsAndCurrency.length())
//				throw new PriceError("交易品定价有误,请检查交易品id:"+goods.getId()+"的定价");
...
//	}
	...
	public GoodsCarryRackService getGoodsCarryRackService() {
		return goodsCarryRackService;
	}
