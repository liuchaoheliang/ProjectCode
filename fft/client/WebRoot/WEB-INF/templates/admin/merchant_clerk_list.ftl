...
<form id="listForm" action="merchant_clerk_list.action" method="post">
      ...
      <table width="99%" border="0" cellspacing="0" cellpadding="0" class="tableA">
       ...
       <#list pager.list as clerk>
         ...
         <tr id="clerk_tr_${clerk.id?c}">
		    <td ><span><a href="clerk_info.action?merchantUserSet.beCode=${(clerk.beCode)!""}" title="查看">${(clerk.beCode)!""}</a></span></td>
	        <td width="15%"><span>
...
$("#addClerk").click(function(){
	window.parent.frames.location.href="to_add_clerk.action?";			
});
function lineDownClerk(id){
	if(window.confirm("确认停用此操作员！")){
		if(id!='' && id != null && id != 'undefined'){
			window.parent.frames.location.href="delete_clerk.action?merchantUserSet.beCode="+id;							
		}else{
			return;
		}						
	}else{
...
function onLineClerk(id){
	if(window.confirm("确认启用此操作员！")){
		if(id!='' && id != null && id != 'undefined'){
			window.parent.frames.location.href="online_clerk.action?merchantUserSet.beCode="+id;							
		}else{
			return;
		}						
	}else{
...
function updateClerk(id){
	if(id!='' && id != null && id != 'undefined'){
			window.parent.frames.location.href="to_update_clerk.action?merchantUserSet.beCode="+id;							
		}else{
			return;
		}	
}
