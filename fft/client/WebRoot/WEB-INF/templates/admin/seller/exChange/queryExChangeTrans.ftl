...
<form id="listForm" action="query_exchange_trans.action" method="post">
    <div class="fromDiv">
        ...
        <#list pager.list as trans>
         <tr id="${trans.id?c}">	        
	        	<#if trans.merchant?exists && trans.merchant.mstoreFullName?exists && trans.merchant.mstoreFullName?length <= 7>
	        		<td width="16%" title="${(trans.merchant.mstoreFullName?html)!""}">
	          			${(trans.merchant.mstoreFullName)!"-"}
	          		</td>
