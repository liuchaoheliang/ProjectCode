      ...
      if (xmlHttp.readyState == 4) {
          if (xmlHttp.status == 0 || xmlHttp.status == 200) {
              ...
              if(xmlHttp.responseText!=null&&""!=xmlHttp.responseText){
              	...
              	document.getElementById("payResult").innerHTML="<h5>"+xmlHttp.responseText+"  交易号为：${orderVO.tran.id?if_exists?c}</h5>";//更新DOM
              }
         }
     }
}
     ...
     <div class="discountBox">
         ...
         <div id="payInfo" class="discountFloat" style="display:none">
            ...
            <#if cashdesk?exists && cashdesk.sellerDepositChannelList?exists>
			 ...
			 <#if sendPoints>    
		         <div id="sendPoints">
					<form action="/sendPointsPage.action" method="post">
					<input type="hidden" name="orderVO.tran.id" value="#{orderVO.tran.id?if_exists}" /><br/>
					<input type="submit" value="去赠送积分"/>
					</form>
				</div>
			</#if>
