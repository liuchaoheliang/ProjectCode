...
<!--会员个人信息菜单结束-->
  <div class="rightBox" id="rightHeight">
    <div class="clear pt5">
 <!--   <form id="pwdUserChangeForm" method="post" action="changePassword.action">  -->
      <div class="inforBox validate" id="formlist">
        ...
        <dl>
          <dt>新密码：</dt>
          <dd>
         	<div><strong> <input type="password" name="password"  class="loginText" id="password4"><u>*</u></strong></div>
          </dd>
        </dl>   
        ...
        <dl>
          <dt>确认新密码：</dt>
          <dd>
         	<div><strong> <input type="password" name="password1"  class="loginText" id="password5"><u>*</u></strong></div>
          </dd>
        </dl>   
        ...
        <div class="ml145 mt5"> 
          ...
          <div id="savePwd" class="textBtn"><B>确定</B></div>
          </a> 
        </div>       
      </div>
  <!--  </form> -->  
...
$('#savePwd').click(function(){
	...
	if(subm){
		...
		var newpwd = $.trim($('#password4').val());
		var newpwd2 = $.trim($('#password5').val());
		if(oldpwd == ''){
			$.layer({
				title:['分分通提示您',true],
				time:3,
		...
		$.getJSON("changePassword.action?oldpassword="+oldpwd+"&password="+newpwd+"&password1="+newpwd2,function(data){
				$.layer({
					title:['分分通提示您',true],
					time:3,
					area : ['auto','auto'],
