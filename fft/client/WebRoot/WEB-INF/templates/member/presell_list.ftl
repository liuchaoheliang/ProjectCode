...
<#include "/WEB-INF/templates/common/user_manage_menu.ftl">
    ...
    <div class="rightBox" id="rightHeight">
        ...
        <form id="listForm" action="presell_trans_list.action" method="post">
            <div class="fromDiv">
         ...
         <#--</span>-->
                <a href="#" class="fl">
                    <div id="searchButton" class="adminBtn"><B>查询</B></div>
                </a>
            </div>
            ...
            <div class="clear pt10">
                ...
                <#list pager.list as trans>
                    ...
                    <tr id="${trans.id?c}">
                        <td>
                            <#if trans.merchant?exists && trans.merchant.mstoreFullName?exists && trans.merchant.mstoreFullName?length <= 4>
                            ${trans.merchant.mstoreFullName?html!""}
                            <#elseif trans.merchant?exists && trans.merchant.mstoreFullName?exists >
                            ${trans.merchant.mstoreFullName?html[0..4]}...
                            <#else>
                                                                       -
                            </#if>
                        </td>
                        <td><#if trans?exists>${trans.id?c}<#else>-</#if>
                        </td>
                        <td title="${(trans.createTime)!""}">${(trans.createTime?date("yyyy-MM-dd"))!"-"}</td>
                        <td>${(trans.consumeTime?date("yyyy-MM-dd"))!"暂未消费"}</td>
                        <td>
                        ...
                        <td>
                            <#if trans.state=="g">
                                <span class="greenFont">${(trans.remark)!"-"}</span>
                            <#elseif  trans.state=="r">
                                <span class="redFont">${(trans.remark)!"-"}</span>
                            <#elseif trans.state=="b">
                                <span class="blueFont">${(trans.remark)!"-"}</span>
                            </#if>
                        </td>
                        <td id="duanxin">
                            <a href="toComplaint.action?complaint.orderId=${trans.id?c}&complaint.toUserId=${(trans.merchant.userId)!""}">投诉</a>
                            <a class="ajaxlinkDetail" href="presell_tran_detail_Info.action?trans.id=${trans.id?c}<#if trans.authTicketId?exists>&authIdElse=${trans.authTicketId}</#if>" title="查看详情">详情</a>
                            <#if trans.isConsume?exists && trans.isConsume == '0' && trans.smsNumber?eval < 4 >
                                <a href="#" onclick="remsg('${trans.id?c}','${trans.authTicketId}')"; ><img src="${base}/template/member/images/duanxin.png" title="重发验证券短信"></a>
                            </#if>
                        </td>
                    </tr>
                </#list>
