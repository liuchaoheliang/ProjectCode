...
$(document).ready( function() {
	...
	$('a.exchangeAffirmBut').click(function() {
	    ...
	    if (payMethod == "09")
	    {
	        ...
	        document.getElementById("exchangeLotteryAffirmForm").target = winName;
	        window.open("", winName);
	    }
	    
		$('#exchangeLotteryAffirmForm').submit()
	});
...
<!--付款确认开始-->
<form id="exchangeLotteryAffirmForm" method="post" action="transExchange.action">
<!-- <input name="exchangeTempBean.goodsRackId" value="${(exchangeTempBean.goodsRackId)!""}" type="hidden">
  ...
  <input name="exchangeTempBean.period" value="${(exchangeTempBean.period)!""}" type="hidden">
  <input name="exchangeTempBean.lotteryValue" value="${(exchangeTempBean.lotteryValue)!""}" type="hidden">
  -->
    ...
    <div class="conborder">
      ...
      <dl>
        ...
        <dd>
          <div> 
          <span>
         		 <#if exchangeTempBean.payMethod?exists && exchangeTempBean.payMethod=="02" >
          			现金(珠海农商银行手机银行卡支付)
	              </#if>
