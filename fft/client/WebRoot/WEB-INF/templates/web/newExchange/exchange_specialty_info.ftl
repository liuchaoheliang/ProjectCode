...
$(document).ready( function() {
	...
	function  selectEvent(){ 
		...
		if(selectPayMethod.val()=="09"){
			...
			showPayMethodHtml = '<div class="userpoints">商品总价：<b>'+eval(cash_value.text())*eval($("#number").val())+'</b>元</div>';
		}
		...
		if(selectPayMethod.val()=="00"){
			...
			showPayMethodHtml = '<div class="userpoints">商品总价：<b>'+eval(fftpoint_value.text())*eval($("#number").val())+'</b>分</div>';
		}
		
		//银行积分
		if(selectPayMethod.val()=="01"){
			var bankpoint_value = $("#bankpoint_value");
			showPayMethodHtml = '<div class="userpoints">商品总价：<b>'+eval(bankpoint_value.text())*eval($("#number").val())+'</b>分</div>';
		}
		...
		if(selectPayMethod.val()=="03"){
			...
			showPayMethodHtml='<div class="userpoints">商品总价：<b>'+eval(fftpoint_fix_value.text())*eval($("#number").val())+'</b>分 + <b>'+eval(fftcash_fix_value.text())*eval($("#number").val())+'</b>元</div> ';
		} 
		...
		if(selectPayMethod.val()=="04"){
			...
			showPayMethodHtml='<div class="userpoints">商品总价：<b>'+eval(bankpint_fix_value.text())*eval($("#number").val())+'</b>分 + <b>'+eval(bankcash_fix_value.text())*eval($("#number").val())+'</b>元</div> ';
		} 
		
		//分分通积分+现金(比例)
		if(selectPayMethod.val()=="05"){
			showPayMethodHtml='<div class="meinput validate" id="formlist"> <strong>使用积分：<input type="text" class="loginText4" id="points"><i>可使用积分：<span id="default_fftpoint">1080-2100</span> 分，您还需要支付现金<span id="default_fftpointCash">200</span> 元</i><span id="myMessage"></span></strong></div>';
			showPayMethod.html(showPayMethodHtml);
			
			var fftpointsRatio_value = $("#fftpointsRatio_value").text();//积分
			
			var fftpointsCashRatio_value = $("#fftpointsCashRatio_value").text();//现金
			
			
			var fftpointsRatio = fftpointsRatio_value.split("-");
			var fftpointsCashRatio = fftpointsCashRatio_value.split("-");
			
			var default_fftpoint = $("#default_fftpoint"); //默认积分
			var default_fftpointCash = $("#default_fftpointCash");//默认现金
			
			var total_cash=parseInt(fftpointsRatio[0])+parseInt(fftpointsCashRatio[0]);//总共的积分加现金
			var myMessage = $("#myMessage");
			
			$("#points").val(fftpointsRatio[0]);//积分值
			if(Math.ceil(parseFloat(fftpointsRatio_value.split("-")[0]).toFixed(2)) > Math.floor(parseFloat(fftpointsRatio_value.split("-")[1])*eval($("#number").val()))){
			default_fftpoint.text('0-0');//使用范围
			
			}else{
				default_fftpoint.text(Math.ceil(parseFloat(fftpointsRatio_value.split("-")[0]).toFixed(2))+"-"+Math.floor(parseFloat(fftpointsRatio_value.split("-")[1])*eval($("#number").val())));//使用范围
			}
			default_fftpointCash.text(eval(fftpointsCashRatio[0])*eval($("#number").val()));
			...
			$("#points").bind("blur", function(){
				...
				if(!regexp_value.test(points_value)){
  				...
  				if(parseInt(points_value)<eval(fftpointsRatio_value.split("-")[0]) || parseInt(points_value)>eval(fftpointsRatio_value.split("-")[1])*eval($("#number").val())){
  					$(this).attr("className","errorText4");
  					...
  					default_fftpointCash.text((total_cash*eval($("#number").val()))-parseInt(points_value));
  				}
			}); 
			return ;
		} 
		...
		if(selectPayMethod.val()=="06"){
			showPayMethodHtml='<div class="meinput validate" id="formlist"> <strong>使用积分：<input type="text" class="loginText4" id="points"><i>可使用积分：<span id="default_bankpoint">1080-2100</span>分，您还需要支付现金<span id="default_bankpointCash">200</span>元</i><span id="myMessage"></span></strong></div>';
			showPayMethod.html(showPayMethodHtml);
			...
			default_bankpoint.text(Math.ceil(parseFloat(bankpointsRatio_value.split("-")[0]).toFixed(2))+"-"+Math.floor(parseFloat(bankpointsRatio_value.split("-")[0])*eval($("#number").val())));//使用范围
			default_bankpointCash.text(eval(bankpointsCashRatio[0])*eval($("#number").val()));
			...
			$("#points").bind("blur", function(){
				...
				if(!regexp_value.test(points_value)){
  				...
  				if(parseInt(points_value)<parseInt(bankpointsRatio[0]) || parseInt(points_value)>eval(bankpointsRatio_value.split("-")[0])*eval($("#number").val())){
  					$(this).attr("className","errorText4");
  					myMessage.html("<h3>请正确输入积分范围</h3>");
  					return;
  				}
  					...
  					myMessage.html("<h2>输入成功</h2>");
  					$("#usePointRaioValue").val(points_value);
  					default_bankpointCash.text(eval(bankpointsCashRatio[0])*eval($("#number").val())-parseInt(points_value));
  				}
			}); 
			return ;
		} 
...
<!--内容开始-->
<form id="exchangeSpecialtyViewForm" method="post" action="exchange_specialty_view.action">
       ...
       <div class="buyBox">
         ...
         <div class="meSelect">
           ...
           <span class="fl" id="uboxstyle"> 
            <!---->
            <select id="selectPayMethod" name="exchangeTempBean.payMethod" style="width:300px;">
