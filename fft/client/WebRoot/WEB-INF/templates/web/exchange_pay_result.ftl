...
<script type="text/javascript">
			...
			function getVisitNum(){
				createXMLHttpRequest(); 
			     ...
			     var url = "query_exchange_pay_result.action?trans.id=${trans.id?if_exists?c}";
			    // createXmlHttp();
			      ...
			      if (xmlHttp.readyState == 4) {
			          if (xmlHttp.status == 0 || xmlHttp.status == 200) {
			              ...
			              if(xmlHttp.responseText!=null&&""!=xmlHttp.responseText){
			              	...
			              	document.getElementById("payResult").innerHTML="<div style='color:red;font-size:16px;margin-top:30px;'><b>"+payResultVal_fromServer+"    订单号为：${trans.id?if_exists?c}</b></div>";//更新DOM
			              }
			         }
			     }
			}
			...
			window.onload = function(){  //加载页面时自动运行
				...
				var transPointsId="${trans.id?if_exists}";
				if(transPointsId==""){
