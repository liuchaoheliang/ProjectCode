...
<div class="box1010 pt10">
			...
			<div id="products">
                <ul class="pagination">
					<#list newMerchantProductPager.list as merchantProduct>
						<li><a data="${merchantProduct.id?c}" href="merchant_product_info.action?id=${merchantProduct.id?c}"><img src="${stack.findValue("@com.froad.util.Command@IMAGE_SERVER_URL")}${(merchantProduct.photoUrl)!""}" onError="this.src='${base}/template/common/images/moren.gif'" width="55" ></a>
				...
				<div class="slides_container" id="pingyi">
					<#list newMerchantProductPager.list as merchantProduct>
						<a data="${merchantProduct.id?c}" href="merchant_product_info.action?id=${merchantProduct.id?c}" target="_blank"><img src="${stack.findValue("@com.froad.util.Command@IMAGE_SERVER_URL")}${(merchantProduct.photoUrl)!""}" onError="this.src='${base}/template/common/images/moren.gif'" width="240px" height="242px"></a>
					</#list>
				</div>	
		    </div>
    ...
    <div class="linkBlack" id="divTab02" style="display:none">
      <div class="ddInfLeft">
     	...
     	<#elseif merchantPresent?exists && merchantPresent.txt?exists >	         		
      			${merchantPresent.txt[0..160]}...
     	<#else>
      			-
        </#if>     
      </div>
      <div class="ddInfRight"><img src="${stack.findValue("@com.froad.util.Command@IMAGE_SERVER_URL")}${(merchantPresent.photoUrl?html)!""}" onError="this.src='${base}/template/common/images/moren.gif'"></div>
    </div>
   <!--tab02结束--> 
        ...
        <ul>
         <!-- 图片列表 begin -->
          <#list merchantPhotoList as merchantPhoto>
	        <li>
	          	<img src="${stack.findValue("@com.froad.util.Command@IMAGE_SERVER_URL")}${(merchantPhoto.photoUrl)!""}" onError="this.src='${base}/template/common/images/moren.gif'"/>
  ...
  <!--热推商品开始--> 
    <div class="hotProduct mt10">
      <dl>
        <dt><B>热推商品</B><span><#if (hotPushMerchantProductPager.list?size > 0)><a href="merchantProuct_by_merchantId_list.action?id=${(merchant.id?c)!""}">更多>></a></#if></span></dt>
        <dd>       
  ...
  <!--广告开始 
  <div class="mt10"><img src="${base}/template/web/img/ad01.png"></div>
  --> 
  <!--广告结束--> 
  </div>
</div>  
