$(document).ready(function ()
{
    ...
    var expAction = ['expRebate.action', 'expExchange.action', 'expGroup.action', 'expPresell.action'];
    var initAction = ['query_trans.action', 'query_exchange_trans.action', 'query_group_trans.action', 'query_presell_trans.action'];
    ...
    function subForm()
    {
        if (isDownload)
        {
            ...
            {
                form.attr("action", expAction[2]);//为导出积分兑换在引用该js
            }
            else
            {
