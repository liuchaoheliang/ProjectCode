 ...
 * 商户表设计
 */
DROP TABLE IF EXISTS `fft_merchant`;
CREATE TABLE `fft_merchant` (
  ...
  `legal_credent_type` varchar(255) DEFAULT NULL COMMENT '法人证件类型',
  `legal_credent_no` varchar(255) DEFAULT NULL COMMENT '法人证件号',
  ...
  `review_staff` varchar(255) DEFAULT NULL COMMENT '复核人员',
  `is_audit` bit(1) DEFAULT b'0' COMMENT '是否审核',
  ...
  `data_state` int(11) NOT NULL COMMENT '数据状态10-创建，20-录入，30-启用，40-停用，50-删除 ',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=100000000 DEFAULT CHARSET=utf8 COMMENT='商户';
