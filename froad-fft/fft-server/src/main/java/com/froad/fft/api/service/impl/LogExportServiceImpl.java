...
public class LogExportServiceImpl implements LogExportService {
	...
	final static Logger logger = Logger.getLogger(LogExportServiceImpl.class);
	
	@Resource(name="logServiceImpl")
	private LogService logService;
	@Override
	public Long addLog(ClientAccessType clientAccessType,
			ClientVersion clientVersion, LogDto logDto) throws FroadException {
	...
	public PageDto findLogByPage(ClientAccessType clientAccessType,
			ClientVersion clientVersion, PageDto pageDto) {
		if(pageDto == null){
			logger.error("参数不能为空");
			return null;
		}
		Page page=loadBy(pageDto);
		PageDto returnPageDto=loadBy(logService.findLogByPage(page));
	...
	private Page loadBy(PageDto pageDto){
		...
		if(pageDto.getPageFilterDto() != null){
			...
			if(pageDto.getPageFilterDto().getFilterEntity() != null){
				Log log=DtoToBeanSupport.loadByLog((LogDto)pageDto.getPageFilterDto().getFilterEntity());
				pageFilter.setFilterEntity(log);
			}
...
private PageDto loadBy(Page page){
		...
		if(page.getPageFilter() != null){
			...
			if(page.getPageFilter().getFilterEntity() != null){
				LogDto logDto=BeanToDtoSupport.loadByLogDto((Log)page.getPageFilter().getFilterEntity());
		...
		List<Log> listLog=page.getResultsContent();
		List<LogDto> listLogDto=new ArrayList();
		for(Log log:listLog){
			listLogDto.add(BeanToDtoSupport.loadByLogDto(log));
		}
		pageDto.setResultsContent(listLogDto);
