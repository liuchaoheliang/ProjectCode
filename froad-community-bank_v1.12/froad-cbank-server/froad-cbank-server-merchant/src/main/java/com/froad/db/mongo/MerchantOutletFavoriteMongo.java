...
public class MerchantOutletFavoriteMongo {
	...
	*/
	 ...
	 * @throws
	 */
	public int addRecvInfo3(String clientId, long memberCode,List<RecvInfo> stoList ) {
		DBObject valueObj =null;
		int result = 0;
		String id=clientId+"_"+memberCode;
		DBObject whereObj = new BasicDBObject();
	...
	*/
	public int removeRecvInfo(String clientId, long memberCode, String recvId) {
		...
		dbo.put("$elemMatch", new BasicDBObject("recv_id", recvId));
		whereObj.put("recv_info",dbo);
		DBObject valueObj = new BasicDBObject();
		valueObj.put("recv_info.$.isenable", "0");
	...
	public int updateDefaultRecvInfo(String clientId, long memberCode,RecvInfo recvInfo) {
		...
		if(recvInfo.getIsEnable()!=null )
		{
			valueObj.put("recv_info.$.isenable", recvInfo.getIsEnable());
		}
		DBObject whereObj = new BasicDBObject();
		whereObj.put("_id",id);
//		whereObj.put("recv_info.recv_id",recvInfo.getRecvId());
	 ...
	 * @param falseDefalut
	 * @returnm
	 * @modify Yaolong Liang
	 */
	public int updateDefaultRecvInfo(String clientId, long memberCode,String truedefault,String falseDefalut,RecvInfo recvInfo) {
		int result = -1;
		String id = clientId+"_"+memberCode;
		DBObject valueObj = new BasicDBObject();
		...
		DBObject whereObj = new BasicDBObject();
		whereObj.put("_id",id);
		whereObj.put("recv_info.type", recvInfo.getType());
		DBObject dbo = new BasicDBObject();
	...
	*/
	public List<RecvInfo> queryRecvInfo(String clientId, long memberCode){
		...
		{
			List<RecvInfo> tList =merchantOutletFavorite.getRecvInfo();
			if( tList != null && tList.size() > 0 ){
				for( RecvInfo recv : tList ){
					if( "1".equals(recv.getIsEnable()) ){
						recvInfoList.add(recv);
					}
				}
			}
		}
	...
	* @throws 
	*/
	public List<RecvInfo> queryDefaultRecvInfo(String clientId, long memberCode,String isDefault,String type){
		boolean flag=true;//收货地址标志 true为收货地址 false为提货地址
		String id = clientId+"_"+memberCode;
		DBObject whereObj = new BasicDBObject();
		List<RecvInfo> recvInfoList = new ArrayList<RecvInfo>();
		RecvInfo rec = null;
...
//		whereObj.put("recv_info.isdefault",isDefault);
	    ...
	    DBObject query2 = new BasicDBObject();
	 	...
	 	query2.put("recv_info.isenable", "1");
	 	if(type.equals("1")){
	 		if(isDefault!=null && isDefault.equals("1")){
	 			query2.put("recv_info.isdefault", isDefault);
	 		}
	 		query2.put("recv_info.type", type);
	 		pipeLine.add(new BasicDBObject("$sort",new BasicDBObject("recv_info.isdefault",-1)));
	 		
	 	}
	 	if(type.equals("2")){
	 		if(isDefault!=null && isDefault.equals("1")){
	 			query2.put("recv_info.isdeliverdeault", isDefault);
	 		}
	 		pipeLine.add(new BasicDBObject("$sort",new BasicDBObject("recv_info.isdeliverdeault",-1)));
	 		flag=false;
	 	}
	    pipeLine.add(new BasicDBObject("$match", query2));
	    pipeLine.add(new BasicDBObject("$sort",new BasicDBObject("recv_info.createtime",-1)));
	    LogCvt.debug("请求参数为="+"clientId:"+clientId+"memberCode:"+memberCode+"isDefault:"+isDefault+"type:"+type);
	 //   Cursor cursor = mongo.getReadDBCollection(MongoTableName.CB_MERCHANT_OUTLET_FAVORITE).aggregate(pipeLine, AggregationOptions.builder().build());	
	    handleCursor(recvInfoList,pipeLine);
        // 如果查询默认地址无数据，则取最后一条记录为默认地址 
        if(isDefault!=null && isDefault.equals("1")){
	    	if(recvInfoList == null || recvInfoList.size()==0){
	    		
	    	  if(flag){
	    		  query2.removeField("recv_info.isdefault");
	    	  }else{
	    		  query2.removeField("recv_info.isdeliverdeault"); 
	    	  }
	   // 		Cursor cursorAll = mongo.getReadDBCollection(MongoTableName.CB_MERCHANT_OUTLET_FAVORITE).aggregate(pipeLine, AggregationOptions.builder().build());
	    		handleCursor(recvInfoList,pipeLine);
	    		//提货默认如果不存在则回显收货默认,收货默认也没有则显示最近一条
	    		List<RecvInfo> recvInfoAllList = new ArrayList<RecvInfo>();
	    		if(!flag){
	    		if(recvInfoList.size()>0&&recvInfoList!=null){
	    			for(RecvInfo recv:recvInfoList){
	    				if(recv.getIsDefault()!=null&&recv.getIsDefault().equals("1")){
	    					recvInfoAllList.add(recv);
	    					return recvInfoAllList;
	    				}
	    			}
	    			recvInfoAllList.add(recvInfoList.get(0));
	    			return recvInfoAllList;
	    		}
	    			}else{
	    				if(recvInfoList.size()>0&&recvInfoList!=null){
...
//	    		if(recvInfoAllList.size() > 0){
//	    			recvInfoList.add(recvInfoAllList.get(recvInfoAllList.size()-1));
//	    		}
	    	}
	    }
...
//		List<RecvInfo> recvInfoList = merchantOutletFavorite.getRecvInfo();
	...
	private void handleCursor(List<RecvInfo> recvInfoList,List<DBObject> pipeLine) {
		Cursor cursor = mongo.getReadDBCollection(MongoTableName.CB_MERCHANT_OUTLET_FAVORITE).
				aggregate(pipeLine, AggregationOptions.builder().build());
		RecvInfo rec;
		List<DBObject>  dbObj = new CursorHandleImpl().handleList(cursor);
        if(dbObj != null){
        	for (DBObject dbObject : dbObj) {
        		String json = JSonUtil.toJSonString(dbObject.get("recv_info"));
...
//		whereObj.put("recv_info.recv_id",recvInfo.getRecvId());
	...
	*/
	...
	/** 默认地址是否存在
	* @Title: isExitsRecvInfoIsDefault
	* @Description: 
	* @author longyunbo
	* @param @param clientId
	* @param @param memberCode
	...
	* @throws 
	*/
	public int isExitsRecvInfoIsDefault(String clientId, long memberCode) {
		// TODO Auto-generated method stub
		String id = clientId+"_"+memberCode;
		DBObject queryObj = new BasicDBObject();
		queryObj.put("_id", id);
		// 商品ID
		queryObj.put("recv_info.isdefault", "1");
		return mongo.getCount(queryObj, MongoTableName.CB_MERCHANT_OUTLET_FAVORITE);
	 ...
	 * @throws
	 */
	public int isExitsDeliverInfo(String id,RecvInfo recvInfo) {
		DBObject queryObj = new BasicDBObject();
		queryObj.put("_id", id);
		// 商品ID
		queryObj.put("recv_info.phone", recvInfo.getPhone());
		queryObj.put("recv_info.consignee", recvInfo.getConsignee());
		queryObj.put("recv_info.isenable", "1");
		queryObj.put("recv_info.type", "2");
		return mongo.getCount(queryObj, MongoTableName.CB_MERCHANT_OUTLET_FAVORITE);
	}
}
