...
public class OrgProcess extends AbstractProcess {
	...
	private MerchantOutletBankMapper outletBankMapper;
	...
	private final String initPhone = "966866";
	...
	public void begin() {
		...
		outletBankMapper = cqSqlSession.getMapper(MerchantOutletBankMapper.class);
		merchantOutletMapper = cqSqlSession.getMapper(MerchantOutletMapper.class);
	...
	@Override
	public void before() {
//		merchantMapperCG.deleteOrgMerchant(Constans.clientId);
		clearOrgRelationMongo();
		orgMapper.deleteByClientId(Constans.clientId);
	}
	...
	public void process() {
		...
		if(Checker.isEmpty(bankOrganizations)){
			LogCvt.error("原重庆bps表tr_bank_organization无数据");
			return;
		}
		LogCvt.info("原重庆bps表tr_bank_organization 共有"+bankOrganizations.size()+"条数据需迁移");
		...
		for(BankOrganization bankOrganization : bankOrganizations){
			...
			MerchantOutletBank outleBank = outletBankMapper.selectByBankOrg(100000000L, bankOrganization.getOrgCode());
			// 获取地区ID
...
//			if (bankOrganization.getOrgCode().startsWith("3000_00") && !bankOrganization.getOrgCode().equals("3000_000000")){
			if (bankOrganization.getOrgName().endsWith("部") && !bankOrganization.getOrgName().endsWith("营业部") && !bankOrganization.getOrgCode().equals("3000_000000")){
				org.setOrgType(false); //部门机构
			} else {
			...
			case 1:
				...
				String merchantId1 = merchantInfo.get("100000001");
			...
			case 2:
				...
				if (org.getOrgType()){
					String merchantId2 = merchantSimpleID.nextId();
					MerchantCG merchant2 = new MerchantCG();
					merchant2.setCreateTime(bankOrganization.getCreateTime());
					merchant2.setMerchantId(merchantId2);
					merchant2.setOrgCode(Constans.filterOrg(paths[1]));
					merchant2.setClientId(Constans.clientId);
					merchant2.setMerchantName(bankOrganization.getOrgName());
					merchant2.setMerchantFullname(bankOrganization.getOrgName());
					merchant2.setMerchantStatus(true);
					merchant2.setAreaId(areaId);
					merchant2.setPhone(initPhone);
					merchant2.setIsEnable(true);
					merchant2.setDisableStatus(MerchantDisableStatusEnum.normal.getCode());
					merchant2.setIsTop(false);
					merchant2.setProOrgCode(Constans.filterOrg(paths[0]));
					merchant2.setCityOrgCode(Constans.filterOrg(paths[1]));
					merchant2.setCountyOrgCode(null);
					merchant2.setContractBegintime(new Date());
					merchant2.setContractEndtime(DateUtils.addYears(new Date(), 1));
					merchant2.setContractStaff("");
					merchant2.setAuditStartOrgCode(Constans.filterOrg(paths[1]));
					merchant2.setAuditEndOrgCode(Constans.filterOrg(paths[0]));
					merchant2.setAuditState(ProductAuditState.passAudit.getCode());
					merchant2.setAuditOrgCode("0");	//待审核机构编号(完成置0)
					// 新增商户
					merchantId2 = addMerchant(merchant2);
					org.setMerchantId(merchantId2);
					...
					outlet2.setMerchantId(merchantId2);
					...
					// 部门机构不生成商户
					org.setMerchantId(merchantSimpleID.nextId());
					org.setOutletId(outletSimpleID.nextId());
				}
			...
			case 3:
				...
				if (org.getOrgType()){
					//获取bank_org->merchant_id->outlet_id关系
					if(Checker.isEmpty(outleBank) || Checker.isEmpty(outleBank.getMerchantOutletId())){
						LogCvt.error("原重庆表fft_outlet_bank找不到 bank_org->outlet_id关系==>银行机构号[org_code:"+bankOrganization.getOrgCode()+"]");
						continue;
					}
					
					String merchantId3 = merchantSimpleID.nextId();
					MerchantCG merchant3 = new MerchantCG();
					...
					String outletId3 = ObjectUtils.toString(outleBank.getMerchantOutletId());
					//从中间表中获取outletId
					String newOutletId3 = outletInfo.get(outletId3);
					org.setOutletId(newOutletId3);
				} else {
					// 部门机构不生成商户
					org.setMerchantId(merchantSimpleID.nextId());
					org.setOutletId(outletSimpleID.nextId());
...
//		}
	...
	private long getAreaIdByOutletBank(MerchantOutletBank outleBank, Map<String, String> areaInfo){
		long areaId = 0;
		if(Checker.isNotEmpty(outleBank)){
			MerchantOutlet merchantOutlet = merchantOutletMapper.selectById(outleBank.getMerchantOutletId());
			String oldAreaId = "";
	...
	/**
	 * delete record from cb_org_relation
	 */
	private void clearOrgRelationMongo(){
		DBObject queryObj = null;
		
		queryObj = new BasicDBObject();
		queryObj.put(FieldMapping.CLIENT_ID.getMongoField(), Constans.clientId);
		mongo.remove(queryObj, MongoTableName.CB_ORG_RELATION);
	}
	 ...
	 */
	private String addMerchant(MerchantCG merchant){

		String resultMerchantId = merchant.getMerchantId(); 
		
		MerchantCG merchantCNT = new MerchantCG();
		...
		MerchantCG queryMerchant = merchantMapperCG.findOneMerchant(merchantCNT);
...
//TODO				MerchantRedis.set_cbbank_bank_merchant_client_id_org_code(merchant);
			}
			
			/* 缓存全部商户 */
//			addMerchantRedis(merchant);
//TODO			MerchantRedis.set_cbbank_merchant_client_id_merchant_id(merchant); 
			
			/* 缓存该客户端下改地区所有商户ID */
//			addAreaMerchantRedis(merchant);
//TODO			MerchantRedis.set_cbbank_area_merchant_client_id_area_id(merchant);
...
//TODO				MerchantRedis.set_cbbank_top_merchant_client_id(merchant);
			}
