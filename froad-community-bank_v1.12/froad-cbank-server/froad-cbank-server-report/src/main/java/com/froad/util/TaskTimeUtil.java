...
public class TaskTimeUtil {
	...
	public static Date getStartDay(Date date, int amount){
		Calendar c = Calendar.getInstance();
		c.setTime(date);
		c.add(Calendar.DAY_OF_YEAR, amount);
		c.set(Calendar.HOUR_OF_DAY, 0);
	...
	public static Date getEndDay(Date date, int amount){
		Calendar c = Calendar.getInstance();
		c.setTime(date);
		c.add(Calendar.DAY_OF_YEAR, amount);
		c.set(Calendar.HOUR_OF_DAY, 23);
	 ...
	 * @return
	 */
    public static Integer skipDateTime(int date, int skipDay) {
    	SimpleDateFormat dateFormat = null;
    	Date tmpDate = null;
        Calendar calendar = null;
        Integer resultDate = null;

        try {
        	dateFormat = new SimpleDateFormat(DateUtil.DATE_FORMAT2);
			tmpDate = dateFormat.parse(String.valueOf(date));
			
			calendar = Calendar.getInstance();
	        calendar.setTime(tmpDate);
	        calendar.add(Calendar.DATE, skipDay);
	        
	        resultDate = Integer.valueOf(dateFormat.format(calendar.getTime()));
