...
public class MerchantDetailMongo {
	...
	public boolean addMerchant(Merchant merchant,
		...
		if (CollectionUtils.isNotEmpty(typeInfoList)) {
			// if(typeInfoList.size() >= 2){
			// typeInfoList.remove(1);
	 ...
	 * 更新商户评价星级
	 * 
	 * 1 - 查询所属门店 2 - 统计计算评价星级(小数点一位四省五入取整) 3 - 更新
	 * 
	...
	public boolean updateMerchantStarLevelByMerchantId(String merchantId)
		...
		if (merchantId != null && !"".equals(merchantId)) {
			...
			if (outletDetailList != null && outletDetailList.size() > 0) {
				...
				result = manager.update(value, where,
						MongoTableName.CB_MERCHANT_DETAIL, COMMAND_SET) > 0 ? true
						: false;
			}
		}
